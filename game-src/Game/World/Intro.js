...
import { atan, Fn, PI2, positionGeometry, uniform, vec3, vec4 } from 'three/tsl'
...
export class Intro
{
    ...
    setText()
    {
        ...
        this.text.textures = new Map([ [ 'zh-CN', labelTexture ] ])
        this.text.updateTexture = () =>
        {
            ...
            context.font = '900 126px "OVWS Intro Chinese", "Noto Sans SC", "Microsoft YaHei", "PingFang SC", sans-serif'
            context.textAlign = 'center'
            context.textBaseline = 'middle'
            context.fillText('点击开始', 672, 205)

            context.beginPath()
            context.moveTo(520, 275)
            context.bezierCurveTo(430, 238, 344, 255, 292, 326)
            context.stroke()
            context.beginPath()
            context.moveTo(292, 326)
            context.lineTo(300, 273)
            context.moveTo(292, 326)
            context.lineTo(344, 314)
            context.stroke()
        ...
        {
            const fontFiles = [
                { file: 'NotoSansSC-119-wght-normal.woff2', unicodeRange: 'U+70B9' },
                { file: 'NotoSansSC-117-wght-normal.woff2', unicodeRange: 'U+51FB' },
                { file: 'NotoSansSC-118-wght-normal.woff2', unicodeRange: 'U+5F00' },
                { file: 'NotoSansSC-116-wght-normal.woff2', unicodeRange: 'U+59CB' },
            ]

            Promise.allSettled(fontFiles.map(async ({ file, unicodeRange }) =>
            {
                const url = new URL(`fonts/${file}`, document.baseURI)
                const fontFace = new FontFace('OVWS Intro Chinese', `url("${url.href}") format("woff2")`, {
                    weight: '100 900',
                    unicodeRange,
                })
                await fontFace.load()
                document.fonts.add(fontFace)
            })).then(this.text.updateTexture)
        }
    }
