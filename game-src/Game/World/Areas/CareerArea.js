...
export class CareerArea extends Area
{
    ...
    setLines()
    {
        ...
        const labels = {
            careerFreelancer: { lines: [ '业余项目' ], width: 240, height: 60 },
            careerHetic: { lines: [ '系统与运维' ], width: 316, height: 60 },
            careerImmersiveGarden: { lines: [ '云与开源' ], width: 340, height: 60 },
            careerIRLTeacher: { lines: [ '电商', '运营' ], width: 268, height: 92 },
            careerOnlineTeacher: { lines: [ '自托管 / RIME', 'AI 工具链' ], width: 332, height: 92 },
            careerUzik: { lines: [ 'OVWS' ], width: 168, height: 60 },
        }

        const createLabelTexture = (data) =>
        {
            ...
            const fontSize = data.lines.length > 1 ? 31 : 37
            context.clearRect(0, 0, canvas.width, canvas.height)
            context.fillStyle = '#ffffff'
            context.font = `700 ${fontSize * 2}px "Noto Sans SC", "Microsoft YaHei", "PingFang SC", "Pally-Bold", "Nunito", sans-serif`
            context.textAlign = 'center'
            context.textBaseline = 'middle'

            data.lines.forEach((text, index) =>
            {
                const lineHeight = fontSize * 2.05
                const y = canvas.height / 2 + (index - (data.lines.length - 1) / 2) * lineHeight
                context.fillText(text, canvas.width / 2, y, canvas.width - 16)
            })
        ...
        for(const group of lineGroups)
        {
            ...
            const label = labels[line.group.userData.texture]
            line.texture = label ? createLabelTexture(label) : this.game.resources[`${line.group.userData.texture}Texture`]
    ...
    update()
    {
        ...
        const yearCurrent = this.year.start
